...
$EshellUI.Logo = ValueEx @{
	...
	'method:Print' = {
		...
		elseif ($EshellUI.Im.WindowsTerminal) {
			...
			Write-Host -NoNewline "$($VirtualTerminal.Colors.Magenta) In $($VirtualTerminal.Styles.Italic)Windows Terminal $($VirtualTerminal.Styles.NoItalic)v$(Get-WindowsTerminalVersion)"
		}
		Write-Host
